...
/// Converts between `CVPixelBuffer` and `PixelBufferMessage`, and to/from
/// encoded images.
enum PixelBufferBridge {
  /// IOSurface-backed and Metal-compatible, so Core AI can hand buffers to the
  ...
  /// No color management: pixel values pass through unchanged, which is what
  ...
  static func create(width: Int, height: Int, pixelFormatType: OSType) throws -> CVPixelBuffer {
    guard width > 0, height > 0 else {
      throw Errors.invalidArgument(
        "Pixel buffer size must be positive, got \(width)x\(height). Resolve dynamic sizes first.")
    }
  ...
  static func copy(_ buffer: CVPixelBuffer) throws -> CVPixelBuffer {
    let result = try create(
      width: CVPixelBufferGetWidth(buffer),
      height: CVPixelBufferGetHeight(buffer),
      pixelFormatType: CVPixelBufferGetPixelFormatType(buffer))
    try write(message(buffer), into: result)
    return result
  }

  static func info(_ buffer: CVPixelBuffer, handle: Int64) -> NativeValueInfoMessage {
    NativeValueInfoMessage(
      handle: handle,
      kind: .image,
      byteCount: Int64(planeLayouts(buffer).reduce(0) { $0 + $1.bytesPerRow * $1.height }),
      width: Int64(CVPixelBufferGetWidth(buffer)),
      height: Int64(CVPixelBufferGetHeight(buffer)),
      pixelFormatType: Int64(CVPixelBufferGetPixelFormatType(buffer)))
  }
