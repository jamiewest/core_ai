...
/// Core Video pixel format types (`kCVPixelFormatType_*`) that Core ML image
/// features use. Any other four-character code works too; see [fourCC].
abstract final class PixelFormat {
  /// `kCVPixelFormatType_32BGRA`: 8-bit B, G, R, A. Core ML's format for
  /// `RGB` and `BGR` image features.
  ...
  /// `kCVPixelFormatType_OneComponent8`: 8-bit grayscale, Core ML's format
  /// for `GRAYSCALE` image features.
  static const int oneComponent8 = 0x4C303038;

  /// `kCVPixelFormatType_OneComponent16Half`: float16 grayscale, Core ML's
  /// format for `GRAYSCALE_FLOAT16` image features.
...
/// An image feature value (`MLFeatureTypeImage`).
///
/// Inputs may be a file, encoded bytes or raw [PixelBuffer] pixels. Outputs
/// are always a [PixelBuffer]. Files and encoded bytes are decoded natively
/// and stretched to the feature's [MLImageConstraint] size and pixel format
/// unless the size is given explicitly.
sealed class ImageInput extends MLFeatureValue {
  const ImageInput();

  /// An image file on disk (PNG, JPEG, HEIC, ...).
  const factory ImageInput.file(
    String path, {
    int? width,
    int? height,
    int? pixelFormatType,
  ...
  const factory ImageInput.encoded(
    Uint8List bytes, {
    int? width,
    int? height,
    int? pixelFormatType,
  ...
  /// Tightly packed pixels of [pixelFormatType].
...
/// An image read from a file when the prediction runs.
@immutable
final class ImageFile extends ImageInput {
  /// Refers to the image at [path].
  const ImageFile(this.path, {this.width, this.height, this.pixelFormatType});

  /// The file path.
  final String path;

  /// The width to resize to; the feature's constraint by default.
  final int? width;

  /// The height to resize to; the feature's constraint by default.
  final int? height;

  /// The pixel format to convert to; the feature's constraint by default.
  final int? pixelFormatType;

  @override
  FeatureValueMessage toMessage() => EncodedImageMessage(
    path: path,
    width: width,
    height: height,
    pixelFormatType: pixelFormatType,
  ...
  @override
  String toString() => 'ImageFile($path)';
}

/// An encoded image (PNG, JPEG, HEIC, ...) decoded when the prediction runs.
@immutable
final class EncodedImage extends ImageInput {
  ...
  @override
  String toString() => 'EncodedImage(${bytes.length} bytes)';
}
...
@immutable
final class PixelBuffer extends ImageInput {
  ...
  /// Converts tightly packed RGBA8888 pixels (as produced by `dart:ui`'s
  /// `Image.toByteData(format: ImageByteFormat.rawRgba)`) into
  ...
  /// `dart:ui`'s `decodeImageFromPixels`. Supports the formats accepted by
